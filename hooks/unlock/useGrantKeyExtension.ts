...
import type { GrantKeyExtensionParams, GrantKeyExtensionResult, OperationState } from "./types";
 ...
 *
 *   if (result.success) {
 *     console.log("Key extension granted");
 *   }
 * };
 * ```
 */
export const useGrantKeyExtension = () => {
  ...
  const grantKeyExtension = useCallback(
    async (params: GrantKeyExtensionParams): Promise<GrantKeyExtensionResult> => {
      if (!wallet) {
      ...
      try {
        ...
        if (!isLockManager) {
          throw new Error(
            "You must be a lock manager to grant key extensions",
          );
        }
