...
const log = getLogger("blockchain:transaction-utils");
...
export const extractTokenIdsFromReceipt = (receipt: any): bigint[] => {
  ...
  try {
    if (!receipt.logs || receipt.logs.length === 0) {
      log.debug("No logs found in transaction receipt");
      return tokenIds;
    }
    ...
    for (const log of receipt.logs) {
      if (log.topics && log.topics[0] === EVENT_SIGNATURES.TRANSFER) {
        try {
          ...
          log.warn("Failed to parse token ID from transfer event", { error });
        }
      }
    }
  } catch (error) {
    log.error("Error extracting token IDs from receipt", { error });
  }
    ...
    for (const log of receipt.logs) {
      if (log.topics && log.topics[0] === EVENT_SIGNATURES.TRANSFER) {
        try {
          ...
          log.warn("Failed to parse transfer event", { error });
        }
      }
    }
  } catch (error) {
    log.error("Error extracting token transfers", { error });
  }
...
export const extractLockAddressFromReceipt = (
  ...
  try {
    if (!receipt.logs || receipt.logs.length === 0) {
      log.warn("No logs found in deployment receipt");
      return null;
    }
    ...
    for (const logEntry of receipt.logs) {
      try {
        ...
        if (parsedLog && parsedLog.name === "NewLock") {
          ...
          log.info("Extracted lock address from NewLock event", {
            lockAddress,
            deployerAddress,
          });
          return lockAddress;
    ...
    for (const logEntry of receipt.logs) {
      if (logEntry.topics && logEntry.topics.length > 1) {
        for (let i = 1; i < logEntry.topics.length; i++) {
          const potentialAddress = `0x${logEntry.topics[i].slice(-40)}`;
          if (
            ...
            log.info("Extracted lock address from log topics fallback", {
    ...
    log.warn("Could not extract lock address from receipt", {
      deployerAddress,
      logCount: receipt.logs.length,
    });
    return null;
  } catch (error) {
    log.error("Error extracting lock address from receipt", {
      error,
      deployerAddress,
    });
    return null;
...
export const analyzeTransactionReceipt = (
  ...
  log.debug("Transaction analysis completed", {
...
export const estimateContractGas = async (
  ...
  try {
    ...
    log.warn("Gas estimation failed", {
      methodName,
      error,
    });
    return null;
